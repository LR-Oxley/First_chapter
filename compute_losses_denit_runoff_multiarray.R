...
k_base_denitrification <- 0.0308 # turnover rate (fraction of the pool lost per year)
t_opt_C <- 28
...
compute_losses <- function(
  ...
  # losses (apply to nitrate part)
  fast_loss_denit  <- (inorg_pool*0.1)  * k_t # denitrification applied to nitrate part
...
# -----------------------------
mineralised_N <- rast(
  paste0("1.9%_min/arctic_min_N_pool_", ssp, "_no_lim_std.tif")
)
total_thawed <- rast(
  paste0("1.9%_min/arctic_total_thawed_after_mineralisation_", ssp, "_no_lim_std.tif")
)
...
writeRaster(
  ...
  paste0("1.9%_min/new/denitrification_loss_cumulative_", ssp, "_no_lim_std.tif"),
  overwrite = FALSE
)

writeRaster(
  rast(cumulative_runoff_list),
  paste0("1.9%_min/new/runoff_loss_cumulative_", ssp, "_no_lim_std.tif"),
  overwrite = FALSE
)

writeRaster(
  rast(fast_pool_list),
  paste0("1.9%_min/new/mineralised_N_pool_", ssp, "_no_lim_std.tif"),
  overwrite = FALSE
)

writeRaster(
  rast(slow_pool_list),
  paste0("1.9%_min/new/thawed_organic_N_pool_", ssp, "_no_lim_std.tif"),
  overwrite = FALSE
)
